...
class DeadValidator(ChainValidator):
    def __init__(self, player_id: int, is_targeted_by_action: bool=True) -> None:
        super().__init__([
    ...
    class __InnerValidator(BaseValidator):
        ...
        def validate(self, game: Game) -> bool:
            if game.player_set.fetch(self.player_id).is_alive():
                if self.is_targeted_by_action:
                    self.error_message = 'This action can only effect dead players... cheater'
                else:
                    self.error_message = 'This action can only be taken while dead'
